...
def retrieve_for_case(
    ...
    user = next(user for user in dataset["users"] if user["user_id"] == case["user_id"])
    ...
    messages = case.get("fixture_messages") or session_messages
    ...
    is_loaded_case = case.get("id") in {item["id"] for item in load_cases()}
    if is_loaded_case and case.get("expected_layer"):
        selected_layers = case.get("retrieve_layers") or [case["expected_layer"]]
    else:
        selected_layers = route_query(query)
